...
- `kvim-syntax`, `kvim-language`, and `kvim-tui` enable no grammar by default. `kvim-language` enables `editor-services` by default; its `--no-default-features` path is grammar-free and headless. Grammar features imply editor services. Test no grammar, `grammar-rust`, and `all-grammars`. Test `kvim-embed` with default, no-default, `worktree`, `grammar-rust`, and `all-grammars`.
