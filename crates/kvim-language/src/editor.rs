...
mod scss;
use crate::server::{declarations_are_valid, marker_is_valid};
mod services;
mod session;
...
pub mod mock;
#[cfg(all(test, feature = "all-grammars"))]
#[path = "editor/tests.rs"]
mod tests;

pub use crate::server::{LanguageServerDeclaration, ServerFormatting};
#[cfg(feature = "grammar-asm")]
...
/// The largest number of grammar-backed adapters in one registry.
...
impl AnalysisInput {
    ...
    /// Returns the complete buffer revision that produced the source.
    #[must_use]
...
impl Analysis {
    /// Returns the complete buffer revision that produced this result.
    #[must_use]
    pub const fn revision(&self) -> BufferRevision {
        self.revision
    }
...
pub trait LanguageAdapter: Send + Sync {
    ...
    fn extensions(&self) -> &'static [&'static str] {
        self.service_profile().extensions()
    }
    ...
    fn file_names(&self) -> &'static [&'static str] {
        self.service_profile().file_names()
    }
    ...
    fn language_names(&self) -> &'static [&'static str] {
        self.service_profile().language_names()
    }
    ...
    ///
    /// The table holds at most [`crate::LANGUAGE_SERVERS_MAX`] declarations, every
    /// identifier is unique inside the adapter, and at most one declaration
    /// formats. Each declaration names at most [`crate::LANGUAGE_ROOT_MARKERS_MAX`]
    ...
    fn language_servers(&self) -> &'static [LanguageServerDeclaration] {
        self.service_profile().language_servers()
    }
