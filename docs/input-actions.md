...
A host repaints after `Composition::WhichKeyBack`: the breadcrumb may have
shortened, or the cleared prefix may have closed the which-key overlay. The
composer performs the pending-sequence step internally; it exposes no general
resolver mutation operation. `WhichKeyOverlay` remains presentation-only
